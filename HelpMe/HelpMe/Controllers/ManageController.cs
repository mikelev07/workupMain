...
namespace HelpMe.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        [HttpPost]
        public async Task<JsonResult> UploadPortfolioMultiAttach(string name)
        {
            User user = db.Users.Include(s => s.Portfolios).Where(n => n.UserName == name).FirstOrDefault();
            var count = user.Portfolios.Count;
            if (user.Id == User.Identity.GetUserId() && count < 10)
            {
                for (var i = 0; i < Request.Files.Count; i++)
                {
                    if (count < 10)
                    {
                        Portfolios attach = new Portfolios();
